...
# ── Load user's trade history and build equity curve ──
def load_user_trades_and_equity(user_email):
    """Fetch all trades for user and reconstruct equity curve"""
    try:
        # Fetch all trades from API
        r = requests.get(f"{API_URL}/trades/user/{user_email}", timeout=5)
        if r.status_code == 200:
            trades = r.json()
            if isinstance(trades, list) and len(trades) > 0:
                # Store trades in session
                st.session_state.trade_log = trades
                
                # Reconstruct equity curve from trades
                equity_history = [{'time': '00:00:00', 'equity': 100000.0}]
                for trade in trades:
                    equity_history.append({
                        'time': trade.get('timestamp', ''),
                        'equity': float(trade.get('balance_after', 100000.0))
                    })
                st.session_state.equity_history = equity_history
            else:
                # No trades yet - initialize with starting equity
                st.session_state.trade_log = []
                st.session_state.equity_history = [{'time': '00:00:00', 'equity': 100000.0}]
        else:
                    ...
                    if username and password:
                        try:
                            ...
                            if data.get("success"):
                                ...
                                # Load user's trade history and equity curve
                                load_user_trades_and_equity(data.get("email"))
                        ...
                        if otp_code and len(otp_code.strip()) == 6:
                            try:
                                ...
                                if result.get("success"):
                                    ...
                                    # Load user's trade history and equity curve
                                    load_user_trades_and_equity(result.get("email"))

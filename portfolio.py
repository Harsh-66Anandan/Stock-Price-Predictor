...
def init_portfolio():
    ...
    # Reconstruct positions and balance from trade_log
    if st.session_state.trade_log:
        reconstruct_positions_from_trades()

def reconstruct_positions_from_trades():
    """Rebuild positions and balance from trade history"""
    positions = {}
    balance = 100000.0
    
    for trade in st.session_state.trade_log:
        action = trade.get('action') if isinstance(trade, dict) else trade.get('Action')
        asset = trade.get('asset') if isinstance(trade, dict) else trade.get('Asset')
        qty = int(trade.get('quantity') if isinstance(trade, dict) else trade.get('Qty', 0))
        price = float(trade.get('price') if isinstance(trade, dict) else trade.get('Price', '0').replace('$', ''))
        total = float(trade.get('total_value') if isinstance(trade, dict) else trade.get('Value', '0').replace('$', ''))
        
        if action == 'BUY':
            balance -= total
            if asset in positions:
                old_qty = positions[asset]['shares']
                old_avg = positions[asset]['avg_price']
                new_qty = old_qty + qty
                new_avg = ((old_qty * old_avg) + total) / new_qty
                positions[asset] = {'shares': new_qty, 'avg_price': new_avg}
            else:
                positions[asset] = {'shares': qty, 'avg_price': price}
        elif action == 'SELL':
            balance += total
            if asset in positions:
                positions[asset]['shares'] -= qty
                if positions[asset]['shares'] == 0:
                    del positions[asset]
...
def render():
    ...
    with chart_c2:
        ...
        # Ensure at least 2 data points for visualization
        if len(df_eq) < 2:
            df_eq = pd.DataFrame([
                {'time': '00:00:00', 'equity': 100000.0},
                {'time': datetime.datetime.now().strftime("%H:%M:%S"), 'equity': total_equity}
            ])
    ...
    # ── Trade Audit Log ───────────────────────────
    st.markdown("<p class='section-title' style='margin-top:30px;'>TRADE AUDIT LOG (All Time)</p>", unsafe_allow_html=True)
    if st.session_state.trade_log:
        ...
        for trade in st.session_state.trade_log:
            if isinstance(trade, dict):
                df_log_data.append({
                    ...
                    'Asset': trade.get('asset', trade.get('Asset', 'N/A')),
                    'Qty': trade.get('quantity', trade.get('Qty', 0)),
                    'Price': f"${float(trade.get('price', trade.get('Price', '0').replace('$', ''))):.2f}",
                    'Value': f"${float(trade.get('total_value', trade.get('Value', '0').replace('$', ''))):.2f}"
                })

...
if "equity_history"    not in st.session_state: st.session_state.equity_history    = [{"time": "00:00:00", "equity": 100000.0}]
...
if not st.session_state.is_authenticated and st.session_state.current_username:
    ...
    try:
        ...
        if data.get("success"):
            ...
            if st.session_state.user_email and "trade_log" not in st.session_state:
                try:
                    import auth
                    auth.load_user_trades_and_equity(st.session_state.user_email)
                except:
                    pass
    except:
